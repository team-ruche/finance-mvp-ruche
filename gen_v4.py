...
<div class="top">
  ...
  <div>
    ...
    <div style="margin-top:8px;display:flex;gap:8px;justify-content:flex-end;align-items:center;flex-wrap:wrap">
      ...
      <input type="file" id="impfile" accept=".csv,.txt,text/csv" style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden">
    </div>
  </div>
</div>
...
function commitRows(list){if(!list||!list.length){toast('Nada para importar neste arquivo.');return;}var seen={};P.mj.forEach(function(r){if(r._imp)seen[dedupKey(r)]=1;});var added=0,dup=0,addedRows=[];list.forEach(function(r){var k=dedupKey(r);if(seen[k]){dup++;return;}seen[k]=1;r.rc='';r.id=nextId++;r._imp=true;r.f=[];if(!r.nm)r.nm='(sem nome)';P.mj.push(r);addedRows.push(r);added++;});
  var jumped='';
  ...
  recomputeFlags();persistImports();markDirty();buildHist();closeImport();if(window.__mjInit){renderActive();}toast(added?(added+' linha(s) importada(s) (Reconciled vazio — exclusivo do Patrick).'+jumped+(dup?(' '+dup+' já constavam, ignoradas.'):'')):'0 novas linhas (todas já haviam sido importadas).');}
...
function initMJ(){document.getElementById('mjq').addEventListener('input',()=>renderMJ());
  ...
  renderLive();renderMJ();}
document.getElementById('impfile').addEventListener('change',e=>{const f=e.target.files[0];if(!f)return;const nm=f.name||'';
  if(/\.pdf$/i.test(nm)){toast('PDF não é lido aqui. Exporte o extrato em CSV/Excel (ou me mande que eu converto).');e.target.value='';return;}
  if(/\.(png|jpe?g|gif)$/i.test(nm)){toast('Imagem não é lida aqui. Exporte o extrato em CSV/Excel.');e.target.value='';return;}
  if(/\.(xls|xlsx|xlsm)$/i.test(nm)){if(typeof XLSX==='undefined'){toast('Leitor de Excel indisponível. Exporte como CSV.');e.target.value='';return;}const rb=new FileReader();rb.onload=()=>{try{const wb=XLSX.read(new Uint8Array(rb.result),{type:'array'});const ws=wb.Sheets[wb.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(ws,{header:1,raw:true,defval:''});(WEBHOOK?routeViaWebhook:routeImport)(rows,nm);}catch(err){toast('Erro ao ler Excel: '+err.message);}};rb.onerror=()=>toast('Não consegui ler o arquivo.');rb.readAsArrayBuffer(f);e.target.value='';return;}
  const rd=new FileReader();rd.onload=()=>{try{const txt=rd.result;const dl=detectDelim((txt.replace(/^﻿/,'').split(/\r?\n/)[0]||''));const rows=parseCSV(txt,dl);(WEBHOOK?routeViaWebhook:routeImport)(rows,nm);}catch(err){toast('Erro ao ler: '+err.message);}};rd.onerror=()=>toast('Não consegui ler o arquivo.');rd.readAsText(f);e.target.value='';});
